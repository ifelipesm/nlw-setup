...
export function HabitsList({date, onCompletedChanged}: HabitsListProps){
  ...
  const isDateInPast = dayjs(date).endOf('day').isAfter(new Date())
    ...
    <div className="mt-6 flex flex-col gap-3">
      {habitsInfo?.possibleHabits.map(habit=>{
        return(
          ...
          group-data-[state=checked]:border-green-500  hover:border-violet-500 
          transition-colors group-focus:ring-2 focus:ring-violet-600 focus:ring-offset-2 
